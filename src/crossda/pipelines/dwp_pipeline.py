...
from ..pipelines.weighting import _weight_from_D, _merge_weighted
...
def DWP(
    ...
    **_ignored,
    ...
    # ── Phase 1: MAP-style CV selection (identical to MAP, no weighting) ──
    best_setup = run_grid_search(
        ...
        seed=seed, nfolds_out=nfolds_out, log_tag="[DWP]", verbose=verbose,
    )
    ...
    dist_info = compute_distance_ci_table(
        ...
        rng_for_index=lambda i: np.random.default_rng(dist_seed + i) if dist_seed is not None else None,
    )
    ...
    for i in range(len(train_sessions)):
        ...
        # est_method threaded (not hardcoded) so the label matches DWP's real
        # estimator (defaults to 'direct'), instead of always saying 'boot_mean'.
        df = distance_fields(dist_info[i], t_id, dist_est_method)
    ...
    session_roles += target_rows(("weighting", "final"), t_id, t_label)
