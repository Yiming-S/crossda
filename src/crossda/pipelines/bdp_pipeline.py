...
from ..pipelines.scoring import _normalize_score_mode, score_session_cv
    ...
    **_ignored,
    ...
    for feat_idx, feat_name in enumerate(feat_names):
        ...
        ci_tbl = compute_distance_ci_table(
            x_list, x_test, dist_type, dist_param,
            scale=scale_for_distance, B=200, alpha=0.05, est_method="boot_mean",
            rng_for_index=lambda i: np.random.default_rng(feat_seed + i) if feat_seed is not None else None,
        )
        ...
        for da_name in da_names:
            ...
            for clf_name in clf_names:
                ...
                try:
                    X_src_A, X_tgt_A = apply_da(da_name, da_control, X_src, X_tgt)
    ...
    if da_method == "none":
        ...
        try:
            src_da, tgt_da = apply_da(da_method, da_ctrl, X_train_final, X_T)
            final_model = clone(clf_instance)
            final_model.fit(src_da, y_train_final)
            y_pred = final_model.predict(tgt_da)
    ...
    for i in range(len(train_sessions)):
        ...
        ci = ci_map.get(i, {})
        df = distance_fields(ci, t_id, "boot_mean")
    ...
    session_roles += target_rows(("selection", "final"), t_id, t_label)
...
def _score_map_style(
    ...
    """
    return score_session_cv(
        map_score, train_sessions, x_list, y_list,
        feat_name, feat_param, da_name, da_control, clf_instance,
        k_sess=map_k_sess, n_repeats=map_n_repeats,
        shuffle_sessions=map_shuffle_sessions, seed=seed, nfolds_out=nfolds_out,
    )
